...
ExecLimit(LimitState *node)
{
	...
	TupleTableSlot *slot;
	PlanState  *outerPlan;
	static const char *LIMIT_STATE_STRINGS[] = {"LIMIT_INITIAL", "LIMIT_RESCAN",
	"LIMIT_EMPTY", "LIMIT_INWINDOW", "LIMIT_SUBPLANEOF", "LIMIT_WINDOWEND",
	"LIMIG_WINDOWSTART"};
	...
	switch (node->lstate)
	{
		...
		case LIMIT_RESCAN:
			...
			// So it doesn't skip the first element
			if (node->position == 0){
			...
			for (;;)
			{
				...
				// printf("NODE POSITION: %i\n", node->position);
				//printf("LIMIT_CONDITION: %s\n", LIMIT_STATE_STRINGS[node->lstate]);
